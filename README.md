...
If you enable `plot_html : y` and provide `plot_radius_cm`, FicTrac will also generate a same-stem `*.html` plot by running `FictracPlotter.py` after tracking completes. This requires a Python interpreter plus the `pandas`, `numpy`, and `plotly` packages to be available on your machine.
